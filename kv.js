...
class kvjs {
  ...
  expire(key, seconds, option = "") {
    ...
    if (option === "NX" && expireTime !== undefined) {
      ...
      option === "GT" &&
      (expireTime === undefined || expireTime <= now + seconds * 1000)
    ) {
      return 0;
    } else if (
      option === "LT" &&
  ...
  expireat(key, timestampSeconds, option = "EX") {
    ...
    if (option === "XX" && existingTtl === -1) {
      ...
      option === "GT" &&
      (existingTtl === -2 || existingTtl <= ttlMillis)
    ) {
  ...
  pexpire(key, ttlMillis, option) {
    if (
      (option === "NX" && this.store.has(key)) ||
      (option === "XX" && !this.store.has(key))
    ) {
      return 0;
    }
    ...
    if (option === "GT" || option === "LT") {
      const existingTTL = this.pttl(key);
      if (
        (option === "GT" && existingTTL >= ttlMillis) ||
        (option === "LT" && existingTTL <= ttlMillis)
      ) {
        return 0;
      }
    }
  ...
  zrange(key, start, stop) {
    const sortedSet = this.store.get(key) || new Map();
    const sortedMembers = Array.from(sortedSet.entries()).sort(
      (a, b) => a[1] - b[1]
    );
  ...
  zrangebylex(key, min, max, options = {}) {
    ...
    let result = sortedMembers.filter(
      (member) => member >= min && member <= max
    );
  ...
  zscan(key, cursor, options = {}) {
    ...
    while (count > 0 && index < sortedMembers.length) {
      if (
        !options.match ||
        new RegExp(options.match.replace("*", ".*")).test(
          sortedMembers[index][0]
        )
      ) {
  ...
  hscan(key, cursor, match = "*", count = 10) {
    const hashMap = this.store.get(key) || new Map();
    const filteredFields = Array.from(hashMap.keys()).filter((field) =>
      field.includes(match)
    );
    ...
    return [
      nextCursor,
      filteredFields
        .slice(cursor, endIndex)
        .map((field) => [field, hashMap.get(field)]),
    ];
  }
